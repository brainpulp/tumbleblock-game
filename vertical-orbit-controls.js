(() => {
  ...
  canvas.addEventListener("pointerdown",event=>{if(animation||cameraSnap)return;const point=localPoint(event),face=[...hitFaces].reverse().find(item=>pointInPoly(point,item.poly));if(face)return;event.stopImmediatePropagation();canvas.setPointerCapture(event.pointerId);orbitPointer={pointerId:event.pointerId,start:point,startBasis:viewBasis,horizontal:0,vertical:0,axis:null,moved:false};},true);
  canvas.addEventListener("pointermove",event=>{if(!orbitPointer||event.pointerId!==orbitPointer.pointerId||animation)return;event.stopImmediatePropagation();const point=localPoint(event),dx=point.x-orbitPointer.start.x,dy=point.y-orbitPointer.start.y;if(Math.hypot(dx,dy)<3)return;orbitPointer.moved=true;orbitPointer.axis ||= Math.abs(dx) >= Math.abs(dy) ? "horizontal" : "vertical";orbitPointer.horizontal=orbitPointer.axis==="horizontal"?-dx/pixelsPerQuarter*quarter:0;orbitPointer.vertical=orbitPointer.axis==="vertical"?-dy/pixelsPerQuarter*quarter:0;viewBasis=orthonormal(basisAt(orbitPointer.startBasis,orbitPointer.horizontal,orbitPointer.vertical));render();},true);
