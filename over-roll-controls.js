(() => {
  ...
  rollCandidates = function(index) {
    ...
    cubes.forEach((neighbor, neighborIndex) => {
      ...
      candidates.push({
        destination,
        turns: 2,
        path: [{ destination, axis: cross(from, overDirection), pivot: neighborCenter, relative: sub(cube.pos.map(n => n + .5), neighborCenter), turns: 2 }],
  ...
  doSlide = function(face) {
    ...
    const roll = rollCandidates(face.cubeIndex).map(candidate => {
      const displacement = sub(candidate.destination, face.cube.pos);
      return { candidate, score: dot(push, displacement) / Math.hypot(...displacement) };
    }).filter(entry => entry.score > .5).sort((a, b) => b.score - a.score || a.candidate.turns - b.candidate.turns)[0]?.candidate;
    if (!roll) return blocked();
    animation = null;
    let orientation = [...cubes[face.cubeIndex].orient];
    for (const step of roll.path) for (let turn = 0; turn < step.turns; turn++) {
    ...
    return animateMove({ index: face.cubeIndex, destination: roll.destination, path: roll.path, turns: roll.turns, orient: orientation, type: "roll", duration: 440, fromProgress: 0, sound: "roll" });
  };
})();
